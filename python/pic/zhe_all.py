...
# 自定义颜色、标签和点样式
colors = ['black', 'black', 'black', 'black', 'blue', 'green', 'red', 'orange']  # All lines black
labels = [r'$\eta$=1', r'$\eta$=2', r'$\eta$=3', r'$\eta$=4', 'RC', 'RJ', 'RE', 'RN']
markers = ['o', 's', '^', 'x', 'p', 'H', 'X', 'P']  # 圆形、方块、三角形、十字形、五边形、六边形、❌、➕
fillstyles = ['none', 'none', 'none', 'none', 'none', 'none', 'none', 'none']  # Hollow markers
...
for i, key in enumerate(results_by_strategy.keys()):
    plt.plot(
        ...
        markersize=20,  # Slightly smaller markers
        linewidth=2,
        color=colors[i],
        label=labels[i],
...
# 设置坐标轴格式
plt.gca().yaxis.set_major_formatter(mticker.FuncFormatter(lambda y, _: f'{y/10000:.3f}'))
plt.gca().yaxis.set_major_locator(mticker.LinearLocator(3))
#plt.ylim(0, 150)
plt.xlim(left= 0.000)  # 设置横轴从0开始
plt.ylim(bottom=0)  # 设置纵轴从0开始

# 设置标签和图例

#plt.xlabel(f'{parameter}', fontsize=25)
plt.xlabel(r'$\mathit{k}$', fontsize=28)  # 使用 \mathit 强制斜体
...
plt.savefig("./1.7_all.pdf", format = 'pdf')
plt.show()
