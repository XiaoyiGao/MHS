...
for k in k_values:
    base_path_param = f'C://Users//Lenovo//Desktop//code//SSA_test//add//k//k_{k}//{model}//'
    temp_results_2 = {
        'random_edge': [],
        'random_neighbor': []
    }
    ...
    for i in range(1, 6):
        folder_name = f'{model}_{i}'
        file_path = os.path.join(base_path_param, folder_name, f'{folder_name}.xlsx')
        ...
        for key in temp_results_2:
            temp_results_2[key].append(df[key].iloc[-1])

    for key in temp_results_2:
        avg = sum(temp_results_2[key]) / len(temp_results_2[key]) if temp_results_2[key] else 0
        results_by_strategy[key].append(avg)
...
plt.xlabel(r'$\mathit{N}$', fontsize=40)  # 使用 \mathit 强制斜体
